...
__authors__  = 'Bo Zhu [AES.py], Bruno Vermeulen [RSA.py], Michani. M. De La Calleja E. [nanodatacrypt.py]'
__version__ = 'v1.0.0'
__github__  = 'https://github.com/d4rk6h05t/nano-data-crypt'
...
def main(argv):
    usage = 'usage: ./nanodatacrypt.py -f yourfile -a [aes/rsa] [ -e (--encrypt) / -d (--decrypt) ] '
    ...
    datacrypt = NanoDataCrypt('yourfile.txt')
    ...
    if options.filename:
        datacrypt.filename = datacrypt.cyphertext_file = options.filename
    else:
        filename = input('[+] :: Enter your filename:')
        datacrypt.filename = datacrypt.cyphertext_file = filename
        ...
        if algorithm != 'aes' and algorithm != 'rsa':
            ...
            if algorithm == 'aes':
                ...
                if operation == 'encrypt':
                    package_plaintext = datacrypt.package_plaintext()
                    file = open(datacrypt.filename+'.cpt','w')
                    for plaintext_line in package_plaintext:
                        for hex_chain in plaintext_line:
                            if hex_chain != '':
                                ...
                                cyphertext = aes.encrypt(hex_plaintext,128)
                                file.write(str(cyphertext)+'\n')
                            else:
                                file.write('\n')
                        file.write(str(NanoDataCrypt.END_LINE)+'\n')
                    file.close()
                    print('[+] :: Successful encrypted file!')
                elif operation == 'decrypt':
                    cyphertext_lines = datacrypt.get_cyphertext_lines()
                    file = open(datacrypt.filename,'w')
                    plaintext_line = list()
                    for cyphertext in cyphertext_lines[0]:
                        cyphertext = cyphertext.strip()
                        if cyphertext != NanoDataCrypt.END_LINE and cyphertext != '':
                ...
                if operation == 'encrypt':
                    ...
                    encrypted = datacrypt.get_cyphertext_message()
                    decrypted = rsa.decrypt(encrypted)
                    datacrypt.file_decrypt(decrypted)
                    print('[+] :: successfully decrypted file!')
                    os.remove(datacrypt.get_cyphertext_file())
    sys.exit()
