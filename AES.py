...
ntime = lambda n: (((n << 1) ^ 0x1B) & 0xFF) if (n & 0x80) else (n << 1)
...
class AES:
    ...
    def change_key(self, master_key):
        self.round_keys = plaintext_to_matrix(master_key)
    ...
    def __inv_mix_columns(self, s):
        for i in range(4):
            u = ntime(ntime(s[i][0] ^ s[i][2]))
            v = ntime(ntime(s[i][1] ^ s[i][3]))
            s[i][0] ^= u
            s[i][1] ^= v
            s[i][2] ^= u
            s[i][3] ^= v
...
if __name__ == "__main__":
    master_key =  int( ('key_secret'.encode('utf-8')).hex(), 16 ) 
    plaintext =   int( ('HelloWorld!'.encode('utf-8')).hex(), 16 ) 
    ...
    ciphertext = int(hex(encrypted), 16)
    decrypted = algorithm_AES.encrypt(ciphertext)
